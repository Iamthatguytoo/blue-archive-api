...
def test_valid_api_key(client, monkeypatch, fake_students_list):
    ...
    monkeypatch.setattr(
        api_key_collection, "find_one", lambda *args, **kwargs: fake_key
    )
    monkeypatch.setattr(api_key_collection, "update_one", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        api_key_collection, "find_one_and_update", lambda *args, **kwargs: fake_key
    )

    monkeypatch.setattr(
        student_collection, "count_documents", lambda *args, **kwargs: 1
    )

    monkeypatch.setattr(
        student_collection,
        "find",
        lambda *args, **kwargs: FakeCursor(fake_students_list),
    )

    res = client.get("/v1/students", headers={"x-api-key": "test-key"})
...
# Activation: python -m pytest tests/v1/test_auth.py
